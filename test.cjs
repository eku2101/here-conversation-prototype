...
const context = vm.createContext({ document: { getElementById: get, querySelector: get, querySelectorAll: selector => selector === '#contact-settings input' ? inputs : [], createElement: () => new Element() } });
...
vm.runInContext("for(let i=0;i<20;i++){ changeAttention(-15,'test'); } render();", context);
assert.equal(get('attention').value, 0);
assert.equal(get('mood').textContent, 'Disconnected');
click('reset'); click('focus-toggle'); send('tiktok');
assert.equal(get('attention').value, 100);
send('mom'); assert.equal(get('attention').value, 95);
vm.runInContext("interact(delivered[0], 'open'); interact(delivered[0], 'open');", context);
assert.equal(get('attention').value, 90);
vm.runInContext("blockedTypes.delete('tiktok');", context);
send('tiktok'); assert.equal(get('attention').value, 75);
assert.equal(get('mood').textContent, 'Distracted');
click('reconnect'); assert.equal(get('attention').value, 85);
assert.equal(get('mood').textContent, 'Positive');
click('reset'); assert.equal(get('attention').value, 100);
console.log('PASS: attention bounds, mood transitions, blocked attention preservation, category opt-out, single open penalty, reconnect, Focus off/on, contact selection, emergency override, all low-priority types, delayed queue release without duplication, and reset.');
